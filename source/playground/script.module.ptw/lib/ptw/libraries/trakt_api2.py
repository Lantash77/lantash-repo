...
class TraktAPI2():
    ...
    def get_people(self, section, show_id, full=False):
        ...
        params = {
            'extended': 'full,images'} if full else None
        try:
        ...
        for bookmark in response:
            if not season or not episode:
                ...
                if bookmark['type'] == 'episode' and int(show_id) == bookmark['show']['ids']['trakt'] and \
                        bookmark['episode']['season'] == int(season) and bookmark['episode']['number'] == int(
                    episode):
                    return bookmark['progress']
    ...
    def rate(self, section, item, rating, season='', episode=''):
        ...
        if rating is None:
            ...
            if season and episode:
                data[TRAKT_SECTIONS[section]][0]['seasons'][0]['episodes'][0].update({
                    'rating': int(rating)})
            elif season:
                data[TRAKT_SECTIONS[section]][0]['seasons'][0].update({
                    'rating': int(rating)})
            else:
                data[TRAKT_SECTIONS[section]][0].update({
                    'rating': int(rating)})
    ...
    def __get_cache_limit(self, media, activity, cached):
        if cached:
            ...
            log_utils.log('Now: %s Last: %s Last TS: %s Cache Limit: %.2fs (%.2fh)' % (
                time.time(), utils2.iso_2_utc(activity), activity, cache_limit, cache_limit / 60 / 60),
                          log_utils.LOGDEBUG)
        ...
        if section == SECTIONS.MOVIES:
            ...
            if season:
                data['shows'][0]['seasons'] = [{
                    'number': int(season)}]
                if episode:
                    data['shows'][0]['seasons'][0]['episodes'] = [{
                        'number': int(episode)}]
        return data
