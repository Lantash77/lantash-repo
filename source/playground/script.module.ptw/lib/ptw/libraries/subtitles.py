...
def get(name, imdb, season, episode):
    try:
        ...
        if data['status'] != '200 OK':
            ...
            control.infoDialog('Open Subtitles Error: ' + str(data['status']), "Open Subtitles ERROR",
                               time=6000)  # control.dialog.ok(control.addonInfo('name'), str('Open Subtitles error[CR]Check login and password in settings[CR][CR]Error: %s'% data['status']), '')
