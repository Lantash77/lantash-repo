...
def solvemedia(data):
    try:
        ...
        post = {}
        f = client.parseDOM(result, 'form', attrs={
            'action': 'verify.noscript'})[0]
        k = client.parseDOM(f, 'input', ret='name', attrs={
            'type': 'hidden'})
        for i in k:
            post.update({
                i: client.parseDOM(f, 'input', ret='value', attrs={
                    'name': i})[0]})
        post.update({
        ...
        return {
            'adcopy_challenge': post['adcopy_challenge'],
...
def recaptcha(data):
    try:
        ...
        if data.startswith('http://www.google.com'):
            url += [data]
        url += client.parseDOM(data, 'script', ret='src', attrs={
