...
class KeepAliveHandler:
    ...
    def _reuse_connection(self, h, req, host):
        ...
        try:
            self._start_transaction(h, req)
            r = h.getresponse()  # note: just because we got something back doesn't mean it  # worked.  We'll check the version below, too.
...
def error_handler(url):
    ...
    for i in (0, 1):
        print
        "  fancy error handling %s (HANDLE_ERRORS = %i)" % (pos[i], i)
        HANDLE_ERRORS = i
        try:
...
def continuity(url):
    ...
    m = md5.new(foo)
    print
    format % ('normal urllib', m.hexdigest())
    ...
    m = md5.new(foo)
    print
    format % ('keepalive read', m.hexdigest())
    ...
    m = md5.new(foo)
    print
    format % ('keepalive readline', m.hexdigest())


def comp(N, url):
    print
    '  making %i connections to:\n  %s' % (N, url)
...
def fetch(N, url, delay=0):
    ...
    for i in lens[1:]:
        ...
        if not i == lens[0]:
            print
            "WARNING: inconsistent length on read %i: %i" % (j, i)
...
def test_timeout(url):
    ...
    DEBUG = FakeLogger()
    print
    "  fetching the file to establish a connection"
    fo = urllib2.urlopen(url)
    data1 = fo.read()
    fo.close()
    ...
    i = 20
    print
    "  waiting %i seconds for the server to close the connection" % i
    while i > 0:
...
def test(url, N=10):
    ...
    print
    print
    "performing continuity test (making sure stuff isn't corrupted)"
    continuity(url)
    print
    print
    "performing speed comparison"
    comp(N, url)
    print
    print
    "performing dropped-connection check"
    test_timeout(url)
