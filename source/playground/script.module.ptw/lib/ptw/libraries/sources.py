...
from ptw.libraries import tvmaze
...
class sources:
        ...
        try:
            ...
            if len(items) > 0:
                if select == '1' and 'plugin' in control.infoLabel('Container.PluginName'):
                    ...
                    return sources().addItem(urllib.quote_plus(title))
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=30):
        ...
        if content == 'movie':
            ...
            for i in sourceDict:
                threads.append(threading.Thread(target=self.getMovieSource,
            ...
            import threading
            for i in sourceDict:
                threads.append(threading.Thread(target=self.getEpisodeSource, args=(
                    title, year, imdb, tvdb, season, episode, tvshowtitle, localtvshowtitle, aliases, premiered, i[0],
                    i[1])))
        ...
        string1 = control.lang(32404).encode('utf-8').decode("utf-8")
        string2 = control.lang(32405).encode('utf-8').decode("utf-8")
        string3 = control.lang(32406).encode('utf-8').decode("utf-8")
        string4 = control.lang(32601).encode('utf-8').decode("utf-8")
        string5 = control.lang(32602).encode('utf-8').decode("utf-8")
        ...
        if control.addonInfo('id') == 'plugin.video.bennu':
            try:
                if progressDialog:
                    progressDialog.update(100, control.lang(30726).encode('utf-8') + "\n" + control.lang(30731).encode(
                        'utf-8'))
    ...
    def sourcesFilter(self):
        ...
        for i in local:
            i.update({
                'language': self._getPrimaryLang() or 'en'})
        ...
        if quality in ['0', '1', '2']:
            filter += [i for i in self.sources if
                       i['quality'] == '1080p' and not 'debrid' in i and not 'memberonly' in i]
        ...
        if quality in ['0', '1', '2', '3']:
            filter += [i for i in self.sources if
                       i['quality'] == '720p' and not 'debrid' in i and not 'memberonly' in i]
        ...
        try:
            if not HEVC == 'true':
                self.sources = [i for i in self.sources if not 'HEVC' in i['label'] or not '265' in i['label']]
        except:
            pass
    ...
    def sourcesResolve(self, item, info=False):
        try:
            ...
            headers = dict(urllib.parse_qsl(headers))
