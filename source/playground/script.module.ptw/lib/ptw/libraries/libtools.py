...
class libmovies:
    ...
    def add(self, name, title, year, imdb, tmdb, range=False):
        if not control.condVisibility('Window.IsVisible(infodialog)') and not control.condVisibility(
                'Player.HasVideo') and self.silentDialog is False:
        ...
        try:
            ...
            lib = \
                [i for i in lib if
                 str(i['imdbnumber']) in id or (i['originaltitle'] == title and str(i['year']) == year)][
                    0]
        except:
            lib = []
...
class libtvshows:
    ...
    def add(self, tvshowtitle, year, imdb, tvdb, range=False):
        ...
        try:
            ...
            lib = [i['title'] for i in lib if str(i['imdbnumber']) in id or (
                    i['title'] == items[0]['tvshowtitle'] and str(i['year']) == items[0]['year'])][0]
    ...
    def update(self, query=None, info='true'):
        ...
        for item in items:
            ...
            try:
                ...
                ep = [x['title'] for x in lib if str(x['imdbnumber']) in id or (
                        x['title'] == item['tvshowtitle'] and str(x['year']) == item['year'])][0]
                ep = control.jsonrpc(
                ...
                ep = [{
                    'season': int(i['season']),
                    'episode': int(i['episode'])} for i in ep]
