...
class TraceLogger(object):
    ...
    def log_trace(self, frame, event, arg):
        ...
        msg = '{pre}{indent}{fname}:{lno}:\n{prespace}{indent}{func}({args})\n'.format(addon=addon_name, pre=pre,
                                                                                       prespace=' ' * (len(pre) - 4),
                                                                                       fname=fname, lno=lno, func=func,
                                                                                       args=sargs,
                                                                                       indent='  ' * fc.co_stacksize)
        xbmc.log(msg, xbmc.LOGDEBUG if self.level is None else self.level)
        if self.use_old_trace and self.__old_trace:
            self.__old_trace(frame, event, arg)  # returns None – the scope shouldn’t be traced

    def start(self):
        """Start traceing."""
        # xbmc.log('[XXX] Starting! ' + str(self.__details), xbmc.LOGINFO)
    ...
    def stop(self):
        """Stop traceing."""
        # xbmc.log('[XXX] Stopping!', xbmc.LOGINFO)
        if self.__runing:
            if self.__details == TRACE_CALL:
