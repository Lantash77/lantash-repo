...
# Test information
movie_info = {'title': 'Pulp Fiction', 'imdb': 'tt1375666', 'aliases': [], 'localtitle': 'Pulp Fiction', 'year': '1994'}
...
def worker_thread(provider_name, provider_source):
    ...
    try:
        ...
        if not getattr(provider_source, 'unit_test', False):
            ...
            # Run movie Call
            url = provider_source.movie(movie_info['imdb'], movie_info['title'], movie_info['localtitle'], movie_info['aliases'], movie_info['year'])
            if url is None:
