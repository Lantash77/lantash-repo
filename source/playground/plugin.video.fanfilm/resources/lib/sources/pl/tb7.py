...
class source:
    ...
    def search(self, title, localtitle, year=''):
        try:
            ...
            for title in titles:
                ...
                self.session.post('https://tb7.pl/mojekonto/szukaj', data=data)

                headers = {'Connection': 'keep-alive', 'Cache-Control': 'max-age=0', 'Origin': 'https://tb7.pl', 'Upgrade-Insecure-Requests': '1', 'DNT': '1', 'Content-Type': 'application/x-www-form-urlencoded',
                    ...
                    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3', 'Referer': 'https://tb7.pl/mojekonto/szukaj/1',
                    'Accept-Language': 'pl-PL,pl;q=0.9,en-US;q=0.8,en;q=0.7', }

                data = {'sort': 'size'}

                self.session.post('https://tb7.pl/mojekonto/szukaj/1', headers=headers, data=data)
                r = self.session.post('https://tb7.pl/mojekonto/szukaj/1', headers=headers, data=data).text
    ...
    def sources(self, rows, hostDict, hostprDict):
        ...
        try:
            if type(rows) == tuple:
                ...
                sources.append({'source': 'tb7', 'quality': quality, 'language': info[0], 'url': rows[0], 'info': 'Moje Pliki', 'direct': True, 'debridonly': False})
                return sources
            for row in rows:
                try:
                    ...
                    sources.append({'source': 'tb7', 'quality': quality, 'language': info[0], 'url': link, 'info': size + ' ' + info2, 'direct': True, 'debridonly': False})
                except:
                    continue
            return sources
        except:
    ...
    def resolve(self, url):
        ...
        content = self.session.post('https://tb7.pl/mojekonto/sciagaj', data=data, headers=headers).text
