...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            for url_single in titles:
                ...
                search_url = urllib.urljoin(self.base_link, self.search_link) % (url_single + " " + year)
                cookies = '; '.join(['%s=%s' % (i.name, i.value) for i in requests.get(self.base_link, verify=False).cookies])
                ...
                result = client.request(search_url.replace(" ", "%20"), cookie=cookies)
                result = client.parseDOM(result, 'div', attrs={
                    'class': 'element'})

                for el in result:

                    found_title = str(client.parseDOM(el, 'div', attrs={
                        'class': 'title'})[0]).lower().replace("_", " ").replace(".", " ").replace("-", " ")
                    if is_anime:
                    ...
                    if len(words) >= 4 or is_anime:
                        if not self.contains_all_words(cleantitle.normalize(cleantitle.getsearch(found_title)), words):
                            continue
                    else:
                        if not self.contains_all_words(cleantitle.normalize(cleantitle.getsearch(found_title)), words) or year not in found_title:
                            continue
                    q = 'SD'
                    if self.contains_word(cleantitle.normalize(cleantitle.getsearch(found_title)), '1080p') or self.contains_word(cleantitle.normalize(cleantitle.getsearch(found_title)), 'FHD'):
                        q = '1080p'
                    elif self.contains_word(cleantitle.normalize(cleantitle.getsearch(found_title)), '720p'):
                        q = 'HD'

                    link = client.parseDOM(el, 'a', attrs={
                        'class': 'link'}, ret='href')[0]
                    transl_type = client.parseDOM(el, 'span', attrs={
                        'class': 'version'})[0]
                    transl_type = transl_type.split(' ')
                    transl_type = transl_type[-1]

                    host = client.parseDOM(el, 'span', attrs={
                        'class': 'host'})[0]
                    host = host.split(' ')
                    host = host[-1]
    ...
    def resolve(self, url):
        try:
            ...
            test5 = client.parseDOM(test4, 'a', attrs={
                'class': 'submit'}, ret='href')[0]
