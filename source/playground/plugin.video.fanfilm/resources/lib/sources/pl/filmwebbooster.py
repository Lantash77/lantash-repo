    ...
    def search(self, title, localtitle, year):
        try:
            ...
            for searchtitle in searchtitles:
                ...
                result = client.parseDOM(result, 'ul', attrs={
                    'class': 'resultsList hits'})
                items = client.parseDOM(result, 'li')
                items = [x for x in items if not x.startswith("<a href")]
                orgtitles = []
                for content in items:
                    try:
                        orgtitle = client.parseDOM(content, 'div', attrs={
                            'class': 'filmPreview__originalTitle'})[0]
                    except:
                        orgtitle = "0"
                        pass
                ...
                years = client.parseDOM(result, 'span', attrs={
                    'class': 'filmPreview__year'})
        ...
        try:
            ...
            for searchtitle in searchtitles:
                ...
                for item in zip(titles, ids, years, orgtitles):
                    ...
                    teststring = cleantitle.normalize(cleantitle.getsearch(searchtitle))
                    words = cleantitle.normalize(cleantitle.getsearch(f_title)).split(" ")
                    if self.contains_all_wors(teststring, words) and str(year) in [str(x) for x in range(int(year)-1, int(year)+1)]:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            for code in zip(content[u'link'], content[u'wersja']):
                ...
                test = requests.post("http://fboost.pl/api/player.php?src=%s" % code[0]).text
                ...
                if "cda.pl" in link:
                    try:
                        ...
                        test = client.parseDOM(response, 'div', attrs={
                            'class': 'wrapqualitybtn'})
