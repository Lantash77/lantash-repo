            ...
            for title in titles:
                ...
                for item in result:
                    try:
                        ...
                        rok = client.parseDOM(result, 'span', attrs={'class': 'year'})[0]
            ...
            for title in titles[0]:
                ...
                for item in result:
                    try:
                        ...
                        words = title.split(" ")
                        if self.contains_all_words(name, words) and str(year) in rok:
                            link = link.replace("/tvshows/", "/episodes/")[:-1] + "-%sx%s" % (season, episode)
                            return link
                    except:
                        continue
            return None
        ...
        try:
            ...
            for item in result:
                try:
                    ...
                    video_link = requests.get(video_link, allow_redirects=False).headers['location']
