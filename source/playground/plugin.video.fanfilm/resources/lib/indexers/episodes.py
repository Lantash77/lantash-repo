...
class seasons:
    def __init__(self):
        ...
        self.tvdb_key = control.setting('TVDb_ApiKey')
    ...
    def tvdb_list(self, tvshowtitle, year, imdb, tvdb, lang, limit='', second_search=False):
        try:
            if imdb == '0':
                try:
        ...
        try:
            ...
            if not seasons and not episodes and not second_search:
                url = self.tvdb_by_query % (urllib.quote_plus(tvshowtitle))
                ...
                tvdb = [x[0][0] for x in tvdb if any(y in x[2] for y in years)][0]
                tvdb = client.parseDOM(tvdb, 'seriesid')[0]
                return self.tvdb_list(tvshowtitle, year, imdb, tvdb, lang, limit='', second_search=True)
    ...
    def seasonDirectory(self, items):
        ...
        for i in items:
            try:
                label = '%s %s' % (labelMenu.encode().decode("utf-8"), i['season'])
                try:
                    if i['unaired'] == 'true':
                        label = '[COLOR darkred][I]%s[/I][/COLOR]' % label.encode().decode("utf-8")
                except:
                    pass
...
class episodes:
    ...
    def trakt_list(self, url, user):
        ...
        for item in items:
            try:
                ...
                itemlist.append({'title': title, 'season': season, 'episode': episode, 'tvshowtitle': tvshowtitle, 'year': year, 'premiered': premiered, 'status': 'Continuing', 'studio': studio, 'genre': genre, 'duration': duration, 'rating': rating,
                                 'votes': votes, 'mpaa': mpaa, 'plot': plot, 'imdb': imdb, 'tvdb': tvdb, 'poster': '0', 'thumb': '0'})
            except:
                pass
        ...
        try:
            ...
            self.list.append(
                {'title': title, 'season': season, 'episode': episode, 'tvshowtitle': tvshowtitle, 'year': year, 'premiered': premiered, 'status': status, 'studio': studio, 'genre': genre, 'duration': duration, 'rating': rating, 'votes': votes, 'mpaa': mpaa, 'director': director, 'writer': writer, 'cast': cast, 'plot': plot, 'imdb': imdb, 'tvdb': tvdb, 'poster': poster, 'banner': banner, 'fanart': fanart, 'thumb': thumb, 'snum':
                    i['snum'], 'enum': i['enum'], 'action': 'episodes', 'unaired': unaired, '_last_watched': i['_last_watched'], '_sort_key': max(i['_last_watched'], premiered)})
        except Exception as e:
            pass
    ...
    def trakt_progress_list(self, url, user, lang):
        ...
        for item in result:
            try:
                ...
                for i in range(0, len(item['seasons'])):
                    if int(item['seasons'][i]['number']) > 0: num_1 += len(item['seasons'][i]['episodes'])
        ...
        threads = []
        import threading
        for i in items: threads.append(threading.Thread(target=self.items_list, args=(i,)))
    ...
    def episodeDirectory(self, items):
        ...
        for i in items:
            try:
                if not 'label' in i: i['label'] = i['title'].encode().decode("utf-8")
