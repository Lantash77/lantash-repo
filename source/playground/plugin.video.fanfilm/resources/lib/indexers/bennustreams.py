...
class indexer:
            ...
            for i in items:
                try:
                    ...
                    result += '<item><title> %01dx%02d . %s</title><meta><content>episode</content><imdb>%s</imdb><tvdb>%s</tvdb><tvshowtitle>%s</tvshowtitle><year>%s</year><title>%s</title><premiered>%s</premiered><season>%01d</season><episode>%01d</episode></meta><link><sublink>search</sublink><sublink>searchsd</sublink></link><thumbnail>%s</thumbnail><fanart>%s</fanart></item>' % (
                        int(i[0]), int(i[1]), i[2], imdb, tvdb, tvshowtitle, year, i[2], i[3], int(i[0]), int(i[1]), thumbnail, fanart)
                except:
                    pass
    ...
    def rotten_list(self, url):
        ...
        u = dom_parser.parse_dom(c, 'div', {'class': 'gray-movie-block'})
        if u:
            r = [(dom_parser.parse_dom(i, 'a', req='href'), dom_parser.parse_dom(i, 'span', {'class': 'subtle'}), dom_parser.parse_dom(i, 'span', {'class': 'tMeterScore'}), dom_parser.parse_dom(i, 'img', req='src'), dom_parser.parse_dom(i, 'p')) for
                 i in u]
            r = [(i[0][0].text, i[1][0].text.replace('(', '').replace(')', ''), i[2][0].text, i[3][0].attrs['src'], i[4][0].text) for i in r if i[0] and i[1] and i[2] and i[3] and i[4]]
        else:
            u = dom_parser.parse_dom(c, 'div', {'id': re.compile('row-index-.+?')})
            r = [(dom_parser.parse_dom(i, 'a', req='href'), dom_parser.parse_dom(i, 'span', {'class': 'subtle'}), dom_parser.parse_dom(i, 'span', {'class': 'tMeterScore'}), dom_parser.parse_dom(i, 'img', req='src'), \
                  dom_parser.parse_dom(i, 'div', {'class': ['info', 'synopsis']})) for i in u]
            r = [(i[0][1].text, i[1][0].text.replace('(', '').replace(')', ''), i[2][0].text, i[3][0].attrs['src'], re.sub('<.+?>', '', i[4][0].text)) for i in r if i[0] and i[1] and i[2] and i[3] and i[4]]
        ...
        if r:
            for i in r:
                try:
                    ...
                    self.list.append({'name': name, 'vip': '0', 'url': url, 'action': 'play', 'folder': False, 'poster': i[3], 'banner': '0', 'fanart': control.addonInfo('fanart'), 'content': 'movies', 'imdb': '0', 'tvdb': '0', 'tmdb': '0',
                                      'title': original_name, 'originaltitle': original_name, 'tvshowtitle': '0', 'year': i[1], 'premiered': '0', 'season': '0', 'episode': '0', 'worker': '0', 'rating': i[2], 'plot': client.replaceHTMLCodes(i[4])})
                except:
                    log_utils.log('Scraping Error: Could not add %s to directory.' % str(original_name), log_utils.LOGERROR)
            return self.list
    ...
    def trakt_list(self, url):
        ...
        u = dom_parser.parse_dom(c, 'div', {'class': 'grid-item'})
        r = [(i.attrs['data-runtime'], dom_parser.parse_dom(i, 'img', {'class': 'real'})) for i in u]
        r = [(i[1][0].attrs['title'], re.findall('\(([^\)]+)', i[1][0].attrs['title'])[0] if '(' in i[1][0].attrs['title'] else '0', i[0], i[1][0].attrs['data-original']) for i in r if i[0] and i[1]]
    ...
    def imdb_list(self, url):
        ...
        u = dom_parser.parse_dom(c, 'div', {'class': 'list_item'})
        r = [(dom_parser.parse_dom(i, 'div', {'class': 'hover-over-image'}), \
              dom_parser.parse_dom(i, 'a', req='href'), \
              dom_parser.parse_dom(i, 'span', {'class': 'year_type'}), \
              dom_parser.parse_dom(i, 'div', {'class': 'item_description'}), \
              dom_parser.parse_dom(i, 'img', req='src'), \
              dom_parser.parse_dom(i, 'span', {'class': 'value'})) \
             for i in u]
        r = [(i[0][0].attrs['data-const'], i[1][1].text,
              re.findall('(\d+)', i[2][0].text)[0], re.sub('<.+?>', '', i[3][0].text),
              i[4][0].attrs['loadlate'] if 'loadlate=' in i[4][0].text else i[4][-1].attrs['src'], \
              i[5][0].text if i[5][0].text else '0.0',
              str(int(re.findall('\((\d+)\s*mins', i[3][0].text)[0]) * 60)) \
    ...
    def tv_info(self, i):
        try:
            ...
            self.meta.append({'imdb': imdb, 'tmdb': '0', 'tvdb': tvdb, 'lang': self.lang, 'user': self.user,
                              'item': {'tvshowtitle': tvshowtitle, 'year': year, 'code': imdb, 'imdb': imdb, 'tvdb': tvdb, 'studio': studio, 'genre': genre, 'duration': duration, 'rating': rating, 'plot': plot}})
        except:
            pass
        ...
        for i in items:
            try:
                ...
                if build:
                    ...
                    if devmode == True:
                        try:
                            cm.append(('Open in browser', 'RunPlugin(%s?action=browser&url=%s)' % (sysaddon, urllib.quote_plus(i['url']))))
                        except:
                            pass
    ...
    def process(self, url, direct=True):
        ...
        try:
            ...
            try:
                ...
                title, year, imdb = re.findall('<tvshowtitle>(.+?)</tvshowtitle>', url)[0], re.findall('<year>(.+?)</year>', url)[0], re.findall('<imdb>(.+?)</imdb>', url)[0]
        ...
        try:
            ...
            from ptw.libraries import filmon
