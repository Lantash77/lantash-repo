...
#Wyjatki do scrapera
            ...
            title, poster, plot, fanart, genre, year, thread = scraper.scraper_check(tytul, type='TV')
            if title == '': title = tytul
            if plot == '': plot = ''
            if poster == '': poster, fanart = scraper.Scrap(tytul, type='TV', poster=True)
            if fanart == '': fanart = default_background
            if genre == '': genre = ''
            if year == '': year = ''
            ...
            addon.addDir(title, link, mode='Browse_Seasons', thumb=str(poster), 
                        ...
                        genre=genre, year=year, plot=plot)
    if thread:
    ...
    if section == 'polecane':
        ...
        for item in result:
            ...
            addon.addDir(str(nazwa), url + str(link), mode='List_Episodes', isFolder= True,
                        thumb=str(img), fanart=pic, page=str(url), 
