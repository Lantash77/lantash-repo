...
class Mega:
    ...
    def _api_request(self, data, param=None):
    ...
    def download_url_folder(self, url, dest_path=None, dest_filename=None,
                            encoding=None):
        """
        Download a file by it's public url
        """
        ...
        folder_id = path[0]
        folder_key = path[1]
        file_id, file_key = self._get_nodes(folder_id, folder_key)
        return self._download_file(
            file_handle=file_id,
            file_key=file_key,
            dest_path=dest_path,
    ...
    def _get_nodes(self, root_folder, shared_enc_key):
        ...
        for node in nodes:
            ...
            file_name = attrs["n"]
            file_id = node["h"]
            #print(file_name, file_id, node["t"])
            if node["t"] == 0:
                return file_id, key
            #print(file_name, file_id, node["t"])
    ...
    def _get_nodes_in_shared_folder(self, root_folder: str) -> dict:
        data = {"a": "f", "c": 1, "ca": 1, "r": 1}
        params = {'n': root_folder}
        return self._api_request(data, params)['f']
    ...
    def _download_file(self,
        ...
        if encoding:
            file_name = file_name.encode(encoding).decode('utf-8')

        proxy = self.options.get('proxy', {})

        input_file = requests.get(file_url, stream=True, proxies=proxy).raw

        total_size_in_bytes = int(input_file.headers.get('content-length', 0))
        progress_bar = self.options.get('progress_bar')

        if progress_bar:
            try:
                progress_bar = tqdm(total=total_size_in_bytes, unit='iB', unit_scale=True, desc=file_name)
            except NameError:
                logger.info('Missing tgdm module for progress bar, progress bar disabled')
                progress_bar = False
